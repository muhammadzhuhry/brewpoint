...
- [ ] **Products** — checked `app/(staff)/products/page.tsx`: bigger rewrite than "swap the hook body". Today, `MOCK_PRODUCTS` is loaded whole and **filtered/paginated entirely client-side** (`.filter()` + `.slice()` in the page itself); the real `searchProducts` service does that server-side (`search`/`categoryId`/`page` query params), so filtering has to move from local JS logic into query params that trigger a refetch. `isAdmin` currently reads `mockCurrentUser.role` — replace with `useCurrentUser()` (built in 3.2). The category filter dropdown currently derives its options from `products.map(p => p.category)` (a name string, from the old mock shape) — the real `Product` type only has `categoryId` (a UUID), so the dropdown needs actual category data (name + id) from `GET /api/v1/categories`, meaning this page has a hard dependency on the **Categories** module below even though Categories is listed after it. Build `useCategories()` (or at least its list query) alongside this, out of order if needed, rather than blocking Products on strict list order. **`tsc` confirms this actually touches 5 files, not just the page** — logged them all below rather than discovering them one at a time
  ...
  - [ ] `components/products/products-table.tsx` — mostly mechanical renames: `stock` → `stockQuantity`, `category` → needs a category name looked up via `categoryId` (not on the product row itself anymore), `price.toFixed(...)` → `Number(price).toFixed(...)` (price is a string now, not a JS number — `.toFixed` only exists on `number`)
  - [ ] `components/products/delete-product-dialog.tsx` and `components/products/product-detail-sheet.tsx` — same mechanical renames, **plus** both reference `txnCount`, a field that doesn't exist on the real `Product` type at all. That's because the soft-vs-hard delete decision (whether a product has been sold before) is made server-side now, inside `deleteProduct` (Part 2.6) — the frontend doesn't need to replicate that logic anymore, just show a generic confirmation and let the API response determine what actually happened
  - [ ] `components/products/product-form-dialog.tsx` — trickiest one: it's driven by `react-hook-form`, whose `defaultValues` and submit-handler types are still built around the old mock shape (`category` as a name string, `stock`/`price` as form strings). Needs `categoryId` (a real UUID, picked from `useCategories()`'s list) instead of a category name, and the submitted values need to match `createProductBodySchema`/`updateProductBodySchema` from `lib/validators/product.ts`, not the old frontend-only `productSchema`
  - [ ] Rewire the page off `useState(MOCK_PRODUCTS)` + local `.filter()`/`.slice()` onto `useProducts({ search, categoryId, page })`, `isAdmin` from `useCurrentUser()`
  - [ ] Create/update/delete as `useMutation` calls (`apiPost`/`apiPut`/`apiDelete`) — **no cache invalidation yet**, that's deliberately 3.4's job (see 3.4's own checklist item for this exact module); for now it's fine if the list doesn't auto-refresh after a mutation and the user has to hit Reload
  - [ ] Wire the "Reload" button (built in Part 1.4) to a real TanStack Query `refetch()` instead of the fake loading-skeleton timer
