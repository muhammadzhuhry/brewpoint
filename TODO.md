...
- [ ] **Categories** — checked `app/(staff)/categories/page.tsx` and its 3 dialogs. Same `productCount`/`sampleProducts`-doesn't-exist-on-the-real-type problem as Products' `txnCount`, but this time it's **not** safe to just drop the fields — `PRD.md` §3 explicitly requires "As an Admin, I want to view a category's detail so that I can see which products belong to it," so the detail view specifically needs real product-membership data, not a simplification. The card grid's summary (count + sample names) is a different story — nothing in the PRD requires every card to show a live count, and computing that for every category on the grid would mean an extra query per card (or a new backend aggregate endpoint that doesn't exist in `TECH_SPEC.md`'s file layout) — dropping that part is the right call, same reasoning as `txnCount`
  ...
  - [ ] `components/categories/category-form-dialog.tsx` — **no changes needed.** Its client-side duplicate-name pre-check (`existingNames: string[]`) still works fine fed from real `categories` data — it's a UX nicety (instant feedback before a round-trip), not a replacement for the backend's own uniqueness check (`createCategory`/`updateCategory` still 409 on duplicates regardless)
  - [ ] `components/categories/delete-category-dialog.tsx` — drop `category.productCount > 0` (doesn't exist on the real type) as the pre-known "blocked" decision. Instead: generic confirmation copy (can't assert the outcome upfront anymore, same principle as `DeleteProductDialog`), plus a new optional `error?: string` prop — if the delete attempt actually gets rejected server-side (`CONFLICT`, category still has products), show that reactively inside the same dialog instead of pre-computing it
  - [ ] `components/categories/category-detail-sheet.tsx` — replace `category.productCount`/`category.sampleProducts` with a **real, on-demand fetch**: call `useProducts({ categoryId: category.id })` inside this component itself (only fires while the sheet is actually open, not eagerly for every card in the grid) to list real member products — this is what actually satisfies the PRD requirement above
  - [ ] `app/(staff)/categories/page.tsx` — `useState(MOCK_CATEGORIES)` → `useCategories()`; grid cards simplified to just name + icon (drop the count/sample/"Empty" badge — no cheap way to compute that per-card without N+1 queries, and PRD doesn't require it at the grid level); create/update/delete as `useMutation` (same pattern as Products — no cache invalidation yet, that's 3.4); delete mutation's error (if `CONFLICT`) passed down to `DeleteCategoryDialog`'s new `error` prop instead of closing the dialog on failure
