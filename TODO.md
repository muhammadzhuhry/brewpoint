...
- [ ] `lib/services/dashboard-service.ts` — `getSalesSummary({ from, to })`: total `totalAmount` and count of `transactions` in range, filtered to `status = "completed"` (excludes voided, per PRD acceptance criteria) and `createdAt` between `from`/`to`. New pattern here: this is the first place summing a column across rows is needed — use Drizzle's `sum()` aggregate helper (not `count()`, which you've used before but only counts rows, not sums a value)
- [ ] Same service — `getBestSellers({ from, to, limit })`: ranks products by total quantity sold in range. New pattern: needs a **join** (`transactionItems` → `transactions`, to filter by the parent transaction's date/status — `transaction_items` itself has no `createdAt` or `status` column) plus **`GROUP BY` + `SUM`** (group by `productId`, sum `quantity`, `orderBy(desc(sum))`, `limit`). Use `transactionItems.productNameSnapshot` for display names — deliberately avoids a second join to `products`, and it's the historically-correct name anyway (a renamed/deleted product shouldn't rewrite past best-seller stats, same snapshotting principle as the receipts themselves)
- [ ] Both — if `from`/`to` aren't supplied, default to "today" (start of day → now), per PRD's "date range defaults to today on load"
- [ ] `app/api/v1/dashboard/summary/route.ts` — GET, admin only, reads `from`/`to` query params
- [ ] `app/api/v1/dashboard/best-sellers/route.ts` — GET, admin only, reads `from`/`to`/`limit` query params
- [ ] Test via `curl` with the seeded data: default (today, likely zeroed unless a checkout was run first — matches the frontend's own empty-state handling), a manually-run `checkout` then confirm it shows up, then `void` that same transaction and confirm the totals drop back out (proves the voided-exclusion filter actually works, not just that the query runs), a wider date range covering the seed data, cashier attempting either route (403)
