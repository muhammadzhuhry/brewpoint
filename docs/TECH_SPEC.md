...
*(Unchanged from v1.0 — the relational model doesn't depend on which language/ORM accesses it.)*
...
// lib/db/schema.ts
import {
  pgTable, pgEnum, uuid, text, numeric, integer, boolean, timestamp,
...
export const adjustmentTypeEnum = pgEnum("adjustment_type", ["increase", "decrease"]);
...
export function middleware(req: NextRequest) {
  ...
  if (!session) {
    return pathname.startsWith("/api")
      ? NextResponse.json({ success: false, error: { code: "UNAUTHORIZED", message: "Not authenticated" } }, { status: 401 })
...
*BrewPoint Technical Specification v2.0 — single Next.js app for MVP, structured to make a future Go backend split a configuration change, not a rewrite.*
