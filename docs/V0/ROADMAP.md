...
**Scope:**
- Repo structure: backend (Go Fiber) and frontend (Next.js) as separate services.
- PostgreSQL running locally via Docker Compose.
- Database schema v1: `users`, `categories`, `products`, `transactions`, `transaction_items`.
- Migration tooling set up (`golang-migrate` or `goose`).
- Basic health-check endpoint (`GET /health`) to confirm backend ↔ database connectivity.
- Base Next.js app shell with routing structure planned (login, POS, products, dashboard).

**Not included yet:** No real feature is usable end-to-end. This phase is purely plumbing.

**Definition of done:** Backend boots, connects to Postgres, migrations run cleanly, and Next.js renders a placeholder page fetching `/health` successfully.
...
**Scope:**
- Login endpoint with username/password, JWT issued on success.
- Password hashing (bcrypt).
- Auth middleware in Fiber protecting all routes except `/login`.
- Role check middleware (`admin` vs `cashier`).
...
- Seed script to create the first admin account (since user creation itself requires being logged in as admin).

**Definition of done:** An admin can log in, create a cashier account, log out, and the cashier can log in with restricted access. Attempting to access admin-only routes as a cashier is blocked both on the frontend and backend.
...
**Scope:**
- Cart state on the frontend (add item, adjust quantity, remove item, clear cart).
- Checkout endpoint: validates stock, creates transaction + transaction items, deducts stock — wrapped in a single DB transaction for atomicity.
...
- Deploy backend + Postgres to a hosting provider (e.g., Railway, Fly.io, or a VPS); deploy frontend to Vercel.
- Environment variable/config management per environment (local, staging, production).
...
- **Redis caching:** Cache the product catalog (cache-aside pattern) and today's dashboard summary, with explicit invalidation on writes (product update, new transaction).
- **Rate limiting:** Apply rate limits to the login endpoint (brute-force protection) and checkout endpoint (abuse protection), using Redis as the counter store.
- **Concurrency hardening:** Load-test simultaneous checkouts on the same product to confirm row-locking (`SELECT ... FOR UPDATE`) prevents overselling under real concurrent load, not just in theory.
...
**Scope:**
- Unit tests for core backend logic (checkout, stock adjustment, auth).
- Integration tests against a test database for critical API endpoints.
- End-to-end tests for the main user flows (login → checkout → dashboard) using Playwright.
- Structured logging (request logs, error logs) instead of print statements.
- Health/readiness endpoints suitable for uptime monitoring.
- GitHub Actions pipeline: run tests → build → deploy automatically on merge to main.
